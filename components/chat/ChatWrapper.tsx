...
const ChatWrapper = ({fileId, isSubscribed}: ChatWrapperProps) => {
  async function getFileData(fileId: number) {
  ...
  const {data, isLoading} = useQuery({
    ...
    refetchInterval: (data) => data?.uploadStatus === 'SUCCESS' || data?.uploadStatus === 'FAILED' ? false : 500,
  })
  ...
  if(data.uploadStatus === 'PROCESSING') return (
